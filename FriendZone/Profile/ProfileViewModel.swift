...
class ProfileViewModel: ProfileViewModelProtocol {
   ...
   static var onStateDidChange: ((State) -> Void)?
    ...
    func deletePost(postID: String, completion: @escaping (Bool?) -> Void) {
        firebaseService.deletePost(postID: postID) { result in
            if result == true {
                completion(true)
            } else {
                completion(false)
    ...
    func viewInputDidChange(viewInput: ViewInput) {
        ...
        case let .tapEditPost(post):
            coordinator?.pushViewController(self, .editPost(self, post))
